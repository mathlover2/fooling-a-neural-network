...
process-waiting-files () {
    ...
    local line fields timestamp tempdatafile endfile 
    for line in ${(f)"$(cat $movedatafile | sort -n)"}
    do fields=(${=${line}})
       tempdatafile=${fields[2]}
       endfile=${fields[3]}
       cat $tempdatafile >> $endfile
    done
    rm /tmp/csv.*
    rm $movedatafile
}
...
process-file () {
    ...
    local fields result endfile fullfilename
    local typefolder
    ...
    case $result in
	("MAIL") endfile="mail.csv" ;;
	("CHAT") endfile="chat.csv" ;;
	("BROWSING") endfile="browse.csv" ;;
	("P2P") endfile="p2p.csv" ;;
	("N/A") endfile=/dev/null ;;
    ...
    then tempdatafile=$(mktemp -p /tmp "csv.XXXXXXXXX")
	 ...
	 echo "$timestamp $tempdatafile $endfile" >> $movedatafile
